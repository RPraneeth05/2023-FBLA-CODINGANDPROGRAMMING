...
<td>${event.endDate.toDate().toLocaleDateString()}</td>
                  ...
                  <td>
                     <!--<div class="button__bar">-->
                        ...
                        <input type="button" onclick="deleteEvent('${
                          event.id
                        }')" value="Delete">
                        <input type="button" onclick="showParticipants('${
                          event.id
...
async function updateQuarters() {
  ...
  if (quarters) {
    // Set the values for each quarter
    document.getElementById("first-quarter-start").value =
      quarters.firstStartDate.toDate().toISOString().slice(0, 10);
    document.getElementById("first-quarter-end").value = quarters.firstEndDate
      .toDate()
      .toISOString()
      .slice(0, 10);
    document.getElementById("second-quarter-start").value =
      quarters.secondStartDate.toDate().toISOString().slice(0, 10);
    document.getElementById("second-quarter-end").value = quarters.secondEndDate
      .toDate()
      .toISOString()
      .slice(0, 10);
    document.getElementById("third-quarter-start").value =
      quarters.thirdStartDate.toDate().toISOString().slice(0, 10);
    document.getElementById("third-quarter-end").value = quarters.thirdEndDate
      .toDate()
      .toISOString()
      .slice(0, 10);
    document.getElementById("fourth-quarter-start").value =
      quarters.fourthStartDate.toDate().toISOString().slice(0, 10);
    document.getElementById("fourth-quarter-end").value = quarters.fourthEndDate
      .toDate()
      .toISOString()
      .slice(0, 10);
  }
...
form.addEventListener("submit", async function (event) {
  ...
  const firstEndDate = new Date(
    document.getElementById("first-quarter-end").value
  );
  const secondStartDate = new Date(
    document.getElementById("second-quarter-start").value
  );
  const secondEndDate = new Date(
    document.getElementById("second-quarter-end").value
  );
  const thirdStartDate = new Date(
    document.getElementById("third-quarter-start").value
  );
  const thirdEndDate = new Date(
    document.getElementById("third-quarter-end").value
  );
  const fourthStartDate = new Date(
    document.getElementById("fourth-quarter-start").value
  );
  const fourthEndDate = new Date(
    document.getElementById("fourth-quarter-end").value
  );
  ...
  if (
    ...
    isValid = false;
    errorMessage =
      "Invalid quarter dates! Ensure that each quarter ends before the next one starts.";
  }
    ...
    if (key.includes("End")) {
      ...
      if (val.toDate().toISOString().slice(0, 10) == today) {
        ...
        document.getElementById(
          "rand9"
        ).innerHTML = `From grade 9, ${randWinners[9].fname} ${randWinners[9].lname} won ${prizes["randomPrize"]}`;
        document.getElementById(
          "rand10"
        ).innerHTML = `From grade 10, ${randWinners[10].fname} ${randWinners[10].lname} won ${prizes["randomPrize"]}`;
        document.getElementById(
          "rand11"
        ).innerHTML = `From grade 11, ${randWinners[11].fname} ${randWinners[11].lname} won ${prizes["randomPrize"]}`;
        document.getElementById(
          "rand12"
        ).innerHTML = `From grade 12, ${randWinners[12].fname} ${randWinners[12].lname} won ${prizes["randomPrize"]}`;

        // Display the information of the top four winners in the desired format
        document.getElementById(
          "pprize1"
        ).innerHTML = `First place: ${topFourUsers[0].fname} ${topFourUsers[0].lname} with ${topFourUsers[0].points} points won ${prizes.prize1}`;
        document.getElementById(
          "pprize2"
        ).innerHTML = `Second place: ${topFourUsers[1].fname} ${topFourUsers[1].lname} with ${topFourUsers[1].points} points won ${prizes.prize2}`;
        document.getElementById(
          "pprize3"
        ).innerHTML = `Third place: ${topFourUsers[2].fname} ${topFourUsers[2].lname} with ${topFourUsers[2].points} points won ${prizes.prize3}`;
        document.getElementById(
          "pprize4"
        ).innerHTML = `Fourth place: ${topFourUsers[3].fname} ${topFourUsers[3].lname} with ${topFourUsers[3].points} points won ${prizes.prize4}`;
      }
    }
  }
...
async function pointsSG() {
  ...
  let tableHTML = `
      <table>
         ...
         <tbody>
            <tr>
               <td style='vertical-align: top'>${generateUserTable(
                 grade9Users
               )}</td>
               <td style='vertical-align: top'>${generateUserTable(
                 grade10Users
...
async function generateReport(email) {
  ...
  // console.log(win)
  console.log(user);
  document.querySelector(".r__name").innerHTML = `${user.fname} ${user.lname}`;
  ...
  document.querySelector(".r__points").innerHTML = user.points + " points";
  // console.log(events);
